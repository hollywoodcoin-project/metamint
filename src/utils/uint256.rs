...
/// 2^256 = 2^64 * 4
/// It's in big-endian notation
#[derive(Clone, Copy)]
pub struct Uint256 (pub [u64; U256_SIZE]);

impl Uint256 {
	pub fn to_radix_string(&self) -> String {
		unimplemented!()
	}

	pub fn from_u64(num: u64) -> Self {
		let mut arr = [0; U256_SIZE];
		arr[0] = num;
		Uint256(arr)
	}
	...
	pub fn max() -> Self {
		Uint256([0xFFFFFFFFFFFFFFFF; U256_SIZE])
	}
...
impl Add for Uint256 {
	type Output = Self;
	fn add(self, other: Self) -> Self { Uint256(bn_op!(add &self.0, &other.0, U256_SIZE)) }
}

impl Sub for Uint256 {
	type Output = Self;
	fn sub(self, other: Self) -> Self { Uint256(bn_op!(sub &self.0, &other.0, U256_SIZE)) }
}

impl Mul for Uint256 {
	type Output = Self;
	fn mul(self, other: Self) -> Self { Uint256(bn_op!(mul &self.0, &other.0, U256_SIZE)) }
}
...
impl Div for Uint256 {
	...
	fn div(self, other: Self) -> Self {
		...
		if n == 1 {
			let (r, _) = bn_op!(short_div &self.0, other.0[0], U256_SIZE);
			return Uint256(r);
		}

		let mut result = [0u64; U256_SIZE];
		...
		let mut u = bn_op!(mul &self.0, &[d], U256_SIZE + 1);
		// (V_(n-1) ... V_1 V_0) = (V_(n-1) ... V_1 V_0) * d
		let v = bn_op!(mul &other.0, &[d], U256_SIZE);
		...
		for i in (0..m + 1).rev() {
			...
			while bn_raw_cmp(&qs, &b) == Ordering::Equal || bn_raw_cmp(&bn_op!(mul &qs, &[v[n - 2]], U256_SIZE), &[u[n + i - 2], rs]) == Ordering::Greater {
				// qs = qs - 1
				qs = bn_op!(sub &qs, &[1], 2);
				// rs = rs + V_(n-1)
			...
			if negative {
				...
				let sum = &bn_op!(add ui, &v, U256_SIZE)[0..n];
				// Replace digits
				for j in 0..n {
					ui[j] = sum[j];
				}
...
impl Div<u64> for Uint256 {
	...
	fn div(self, other: u64) -> Self {
		...
		let (q, _) = bn_op!(short_div &self.0, &other, U256_SIZE);
		Uint256(q)
	}
}
